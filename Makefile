...
KIND_CLUSTER ?= oadp-vm-file-restore-test-e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name oadp-vm-file-restore-builder
	$(CONTAINER_TOOL) buildx use oadp-vm-file-restore-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm oadp-vm-file-restore-builder
	rm Dockerfile.cross
...
update-velero-manifests: ## Update Velero manifests used by VMFR, from OADP_OPERATOR_PATH
ifeq ($(OADP_OPERATOR_PATH),)
	$(error You must set OADP_OPERATOR_PATH to run this command)
endif
	@{ \
	VMFR_VELERO_VERSION=$(shell grep -I 'github.com/openshift/velero' $(shell pwd)/go.mod | awk -F' ' '{print $$5}') ;\
	OADP_VELERO_VERSION=$(shell grep -I 'github.com/openshift/velero' $(OADP_OPERATOR_PATH)/go.mod | awk -F' ' '{print $$5}') ;\
	sed -i "s%$$VMFR_VELERO_VERSION%$$OADP_VELERO_VERSION%" $(shell pwd)/go.mod ;\
